...
def answer_call():
    ...
    recording_status_callback='https://0b3a-202-142-106-83.ngrok-free.app/handleRecord',
    timeout=30,
    channels=2
    )
...
def handleRecord():
    ...
    r = requests.post("https://api.devrev.ai/artifacts.prepare" , {"file_name":filename} , config)
    id=r.id
    form = r.formdata
    link=r.url
    formdata={}
    ...
    with open(f'./{filename}' , "rb") as file:
        print(requests.post(link , data=formdata , files={file}))
